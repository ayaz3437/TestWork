...
declare module '@agoric/ertp' {
  type InterfaceSpec = import('@endo/marshal').InterfaceSpec;
  type GetInterfaceOf = import('@endo/marshal').GetInterfaceOf;
  /**
   ...
   * by that key.
   */
  type AmountValue = NatValue | any[] | CopySetValue | CopyBagValue;
  /**
   ...
   * resolution.
   */
  type IssuerGetAmountOf<K extends AssetKind = AssetKind> = (
   ...
   */
  type ShutdownWithFailure = (reason: Error) => void;
  /**
  ...
  type NatMathHelpers = MathHelpers<NatValue>;
  type SetValue = Array<Key>;
  type SetMathHelpers = MathHelpers<any[]>;
  type CopySetValue = CopySet<Key>;
  type CopySetMathHelpers = MathHelpers<CopySetValue>;
  type CopyBagValue = CopyBag<Key>;
  type CopyBagMathHelpers = MathHelpers<CopyBagValue>;
  type AssertAssetKind = (allegedAK: AssetKind) => void;
}
